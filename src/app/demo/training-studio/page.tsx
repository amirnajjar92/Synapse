...
export default function TrainingStudio() {
  ...
  useEffect(() => {
    ...
    // Set first client as selected and load mock messages
  ...
  useEffect(() => {
    // DEMO MODE: Use mock messages only, don't fetch from API
    if (selectedClient) {
      setChatMessages(MOCK_MESSAGES_BY_CLIENT[selectedClient] || []);
    } else {
      setChatMessages([]);
    }
  }, [selectedClient]);
  ...
  const handleSendChat = useCallback(async () => {
    ...
    // DEMO MODE: Add message to state only (no API call)
  ...
  }, [chatInput, selectedClient]);

  const handleDeleteChat = useCallback(async (messageId: string) => {
    // DEMO MODE: Delete message from state only (no API call)
        ...
        <div className="absolute inset-0 z-10 flex flex-col" style={{ backgroundColor: '#0b0b0b4D' }}>
          ...
          <div className="flex-shrink-0 px-4 pb-2">
            <div className="flex bg-white/5 rounded-xl p-1 gap-1">
              {[
                { id: 'dashboard' as Tab, label: 'Dashboard', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
                { id: 'builder' as Tab, label: 'Build Plan', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01' },
                { id: 'repository' as Tab, label: 'Repository', icon: 'M5 19a2 2 0 01-2-2V7a2 2 0 012-2h4l2 2h4a2 2 0 012 2v1M5 19h14a2 2 0 002-2v-5a2 2 0 00-2-2H9a2 2 0 00-2 2v5a2 2 0 01-2 2z' },
                { id: 'messages' as Tab, label: 'Messages', icon: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z' },
              ].map(tab => {
